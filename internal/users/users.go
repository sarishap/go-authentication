...
//GetUserId check if a user exists in database by given username
...
func (user *User) UpdatePassword() {
	statement, err := database.Db.Prepare("UPDATE Users SET Password = $1 WHERE Username = $2")
	if err != nil {
		log.Fatal(err)
	}
	hashedPassword, err := HashPassword(user.Password)
	...
	_, err = statement.Exec(hashedPassword, user.Username)
	if err != nil {
		log.Fatal(err)
	}
}
